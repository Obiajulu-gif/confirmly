# Confirmly — Roadmap Issues

Nine issues ready to paste into `github.com/devfoma/confirmly/issues/new`.
Each block: **title**, **labels**, then the body below the rule.

Suggested labels to create first: `P0` `P1` `P2` · `infrastructure` `payments`
`whatsapp` `ai` `observability` `testing`

---

## 1. Route inbound WhatsApp webhooks by phone_number_id (bring-your-own-number)

**Labels:** `P0` `whatsapp` `infrastructure`
...
## 2. Move rate limiting to Redis — the in-memory limiter does not work on Vercel
...
## 3. Replace after() with a durable queue — dropped replies are currently silent

**Labels:** `P0` `infrastructure` `whatsapp`
...
## 4. Implement refunds and chargebacks
...
## 5. Production Monnify: real settlement events, not just reconciliation
...
- Subscribe to and handle the settlement-completion webhook with the same HMAC verification as transactions
...
## 6. Add observability: Sentry, correlation IDs, and payment alerting

**Labels:** `P1` `observability`
...
## 7. Reduce AI cost and latency: cache extractions, short-circuit obvious cases
...
2. **Short-circuit** before calling NIM:
   ...
   - Anything already handled by `preprocessCommerceMessage` never reaches NIM
...
## 8. Test the money edge cases: overpayment, partial payment, and confirm/pay races

**Labels:** `P1` `testing` `payments`
...
## 9. Ship the growth features the schema already supports
...
Each is independently shippable — split into separate issues if that suits the board better.
