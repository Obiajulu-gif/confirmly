...
export const metadata = {
  ...
  description:
    "Choose a store, tell us who you are once, then order from WhatsApp like you always do.",
};
...
export default async function StartOrderPage({
  ...
  const merchant = store
    ? await prisma.merchant.findFirst({
        where: {
          storeCode: store.replace(/-/g, "").toUpperCase(),
          active: true,
        },
      })
    : null;
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-night-900 text-white">
      ...
      <main className="relative z-10 mx-auto w-full max-w-2xl flex-1 px-4 pb-16 pt-6 sm:pt-10">
        {merchant ? (
          ...
          >
            ...
            <p className="mt-2 text-sm text-white/55">
              Every store below takes orders on the shared Confirmly WhatsApp
              line. Pick one to get started.
            </p>
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {merchants.map((m) => (
                <Link
                  key={m.id}
                  href={`/start?store=${m.storeCode}`}
        ...
        >
          {["Choose a store and say who you are", "Open WhatsApp and chat", "Pay securely, get a verified receipt"].map(
            (label, i) => (
              <li
                key={label}
